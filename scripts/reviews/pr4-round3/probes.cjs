...
const { HEAD, BASE, readSource } = require('./source.cjs');
console.error(JSON.stringify({mode: process.env.ASCLI_REVIEW_SNAPSHOT_DIR ? 'historical-snapshots' : 'pinned-git-objects', head: HEAD, base: BASE, scope: 'isolated components; no Apple network'}));
...
(async()=>{
 await probe('F1',async()=>{
   ...
   for (const tool of ['appstore_respond_to_review','appstore_delete_review_response']) {
     ...
     results.push({tool,confirmedApp:'APP_A',fixtureActualApp:actualApp,gate,bindingReads,reads,writes});
   }
   return results;
 });
 await probe('F2',async()=>{
   ...
   try {
    ...
    for(const replaceExisting of [false,true]){
      ...
      const dryAsc=dry.steps.filter(x=>x.method!=='GET').map(x=>({method:x.method,path:x.path,body:x.body}));
      ...
      results.push({replaceExisting,dryNetworkWrites,dryMethods:dry.steps.map(x=>x.method),actualAppleMethods:asc.map(x=>x.method),unlistedUploads:s3.map(x=>({method:x.method,path:x.path,bytes:x.body.length/2})),appleBodiesMatchAfterIdSubstitution:true});
    }
    return results;
 ...
 await probe('F2cpp',async()=>{
   ...
   try {
    ...
    const asc=trace.filter(x=>x.method!=='PUT');assert.equal(dry.steps.length,asc.length);
    let patch=0;
    for(let i=0;i<asc.length;i++){
      let planned=JSON.stringify(dry.steps[i]).replaceAll('{localeId}','LOCALE').replaceAll('{screenshotSetId}','SET');
      if(dry.steps[i].method==='PATCH')planned=planned.replaceAll('{reservedId}','SHOT'+(++patch));
    ...
    return{cliDryNetworkWrites:0,images:2,appleRequestsMatched:asc.length,omittedUploadPuts:2,
 ...
 await probe('F3',async()=>{
   ...
   const found=await head.getAllPages('/apps/A/appEvents',{'filter[id]':'E1001'});
   const gate=await safety.bindConfirm('appstore_delete_event',{eventId:'E1001'},'A',{get:head.get,getAll:async()=>found});
   ...
   assert.equal(found.length,1000);assert.equal(old.length,1001);assert.equal(gate.ok,false);
   return{head:{returned:found.length,gets:hp.count(),resolvedWithoutError:true,targetFound:found.some(x=>x.id==='E1001')},main:{returned:old.length,gets:bp.count()},eventGate:gate,note:'This is fail-closed for event ownership, not a cross-app write bypass.'};
 });
 await probe('F4',async()=>{
   ...
   return{postEmptyBodyStatus:invalidBody.status,postEmptyDataStatus:emptyData.status,illegalIncludeInParamsStatus:queryParams.status,illegalIncludeInUrlAccepted:urlQuery===undefined,note:'No claim that Apple was called; {data:{}} omits the required type and attributes in Apple\'s published Data schema.'};
 });
 await probe('F5',async()=>{
   ...
   assert.equal(auth.code,4);assert.equal(list.code,3);assert.equal(write.code,3);
 ...
 await probe('F6',async()=>{
   ...
   assert.notEqual(old,head);assert.equal(oldNet.getAttempts(),1);assert.equal(headAttempts,4);
